  ...
  const Trading: React.FC = () => {
    ...
    const fetchListings = async () => {
      try {
            ...
            if (metadata?.data?.uri) {
              if (metadata.data.uri.startsWith('data:application/json;base64,')) {
                ...
                try {
                  ...
                  if (response.ok) jsonMetadata = await response.json();
                } catch (err) {
                  console.error('Error fetching IPFS metadata:', err);
                }
              } else if (metadata.data.uri.startsWith('http')) {
                try {
                  const response = await fetch(metadata.data.uri);
                  if (response.ok) jsonMetadata = await response.json();
                } catch (err) {
                  console.error('Error fetching HTTP metadata:', err);
                }
              }
            }
        ...
        if (formattedListings.length > 0) {
          const totalVolume = formattedListings.reduce((sum: any, listing: any) => sum + (listing.price * listing.credits), 0);
          const avgPrice = formattedListings.reduce((sum: any, listing: any) => sum + listing.price, 0) / formattedListings.length;
    ...
    const currentMarketStats = React.useMemo(() => {
      if (useRealData && realListings.length > 0) {
        const totalVolume = realListings.reduce((sum: number, listing: any) => sum + (listing.price * listing.credits), 0);
        const avgPrice = realListings.reduce((sum: number, listing: any) => sum + listing.price, 0) / realListings.length;
    ...
    const handleBuy = async () => {
      ...
      try {
        ...
        const result = await buyCarbonCredit(connection, wallet, selectedAsset.mint);

        addToast(`Successfully purchased ${selectedAsset.name}! TX: ${String(result?.signature ?? '').slice(0, 8)}...`, 'success');
        setBuyAmount('');
        fetchListings();
      } catch (error) {
    ...
    const handleListNFT = async () => {
      ...
      try {
        ...
        const ownership = await checkNFTOwnership(connection, wallet, listMintAddress);

        if (!ownership?.owned) {
          addToast(ownership?.error || 'You do not own this NFT. Cannot list for sale.', 'error');
          return;
        }
        ...
        const result = await listForSale(connection, wallet, listMintAddress, priceInSOL);

        addToast(`Successfully listed NFT! Listing: ${String(result?.listing ?? '').slice(0, 8)}...`, 'success');
        setListMintAddress('');
        setListPrice('');
                              ...
                              <div className="flex justify-between items-start">
                                ...
                                <div className="text-right">
                                  <p className="text-white font-bold">
                                    {Number(asset.price).toFixed(3)} {useRealData && realListings.length > 0 ? 'SOL' : 'ETH'}
                                  </p>
                      ...
                      <div className="mb-6">
                        ...
                        <div className="flex justify-between text-sm bg-gray-900/50 p-3 rounded-lg">
                          ...
                          <span className="text-white font-bold">
                            {Number(selectedAsset.price).toFixed(3)} {useRealData && realListings.length > 0 ? 'SOL' : 'ETH'}
                          </span>
                        </div>
                      </div>
