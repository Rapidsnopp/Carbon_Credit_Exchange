...
export const getProgram = (connection, wallet) => {
  ...
  try {
    ...
    const program = new anchor.Program(idlWithoutMetadata, PROGRAM_ID, provider);
...
export const mintCarbonCredit = async (
  ...
  if (!carbonData.projectName || !carbonData.projectId || !carbonData.vintageYear) {
  ...
  const onChainCarbonData = {
    ...
    metricTons: new anchor.BN(carbonData.metricTons || carbonData.creditAmount || 1),
    validator: trimAndLimit(carbonData.validator || carbonData.certificationBody || 'Unknown', 32),
    standard: trimAndLimit(carbonData.standard || carbonData.creditStandard || 'VCS', 16),
    projectType: trimAndLimit(carbonData.projectType || 'Reforestation', 32),
    country: trimAndLimit(carbonData.country || carbonData.projectLocation || 'Unknown', 32),
  };
  ...
  try {
    ...
    return {
      mint: mint.toBase58(),
      signature: tx,
      explorerUrl: `https://explorer.solana.com/tx/${tx}?cluster=${import.meta.env.VITE_SOLANA_NETWORK || 'devnet'}`,
      mintExplorerUrl: `https://explorer.solana.com/address/${mint.toBase58()}?cluster=${import.meta.env.VITE_SOLANA_NETWORK || 'devnet'}`,
    };
  ...
  try {
    ...
    if (!tokenAccountInfo) {
      return { 
        ...
        error: 'Token account does not exist. You have not minted or received this NFT.' 
      };
    }

    // Check balance
    const tokenAccountData = await connection.getTokenAccountBalance(tokenAccount);
 ...
 */
export const listForSale = async (connection, wallet, mintAddress, priceInSOL) => {
  if (!wallet.publicKey) {
    throw new Error('Wallet not connected');
  }
  ...
  const priceInLamports = new anchor.BN(priceInSOL * anchor.web3.LAMPORTS_PER_SOL);
 ...
 */
export const retireCarbonCredit = async (connection, wallet, mintAddress, beneficiary) => {
  if (!wallet.publicKey) {
    throw new Error('Wallet not connected');
  }
  ...
  const beneficiaryPubkey = beneficiary ? new PublicKey(beneficiary) : wallet.publicKey;
